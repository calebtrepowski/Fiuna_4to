...
def gauss_seidel(A,b,iter_max,epsilon):
  cortar = False
  m = np.size(A,0)
  n = np.size(b,0)
  #n:numero de filas de b
  ...
  if not(cortar):
    ...
    for i in range(0,iter_max):
      for j in range(0,m):
        ...
        pos = np.array( range(0,n), dtype=int )
...
A = np.array([[9,2,3],[5,14,8],[1,5,17]],dtype=float)
b = np.transpose(np.array([[37,29,12]],dtype=float))

print(gauss_seidel(A,b,iter_max,epsilon))